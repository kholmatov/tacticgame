...
class Gallery extends Widget
{
    ...
    public function renderItem($item)
    {
        ...
        $photo_id = ArrayHelper::getValue($item, 'photo_id');
        $url = ArrayHelper::getValue($item, 'url', $src);
        $options = ArrayHelper::getValue($item, 'options', []);
        Html::addCssClass($options, 'gallery-item '.$photo_id);
        //print_r($options);
        return Html::a(Html::tag('span','x',['class' => 'remove-photo','data-id'=>$photo_id,'data' => [
                'toggle' => 'modal',
                'target' => '#userModal',
            ]]).Html::img($src), $url, $options);
    }
