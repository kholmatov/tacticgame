...
class RedsysWDG extends Widget
{
    ...
    public function getFormDataJsonDemo($id="5555",$amount="6000",$language="002", $pDesc=""){
        $result=RedsysWDG::setMyParameterDemo($id,$amount,$language,$pDesc);
        return json_encode($result);
    }
    ...
    public function setMyParameterDemo($id,$amount,$language,$pDesc){
        if(strlen($pDesc) > 125) $pDesc = substr($pDesc,0,122).'...';
        $miObj = new RedsysAPI();
        //Se Rellenan los campos
        $miObj->setParameter("DS_MERCHANT_AMOUNT",$amount);
