<?php
/**
 * @copyright Copyright &copy; Erkin Kholmatov,
 * https://github.com/kholmatov,
 * ekholmatov@gmail.com - 2015
...
class RedsysWDG extends Widget
{
    ...
    public function getFormData($id="5555",$amount="6000",$language="002", $pDesc="",$urlOK=0, $urlKO=0,$NotificationURL = 0){
        $result=RedsysWDG::setMyParameter($id,$amount,$language,$pDesc,$urlOK, $urlKO, $NotificationURL);
        return '<html lang="es">
            <head>
            </head>
            <body>
    ...
    public function getFormDataJson($id="5555",$amount="6000",$language="002", $pDesc="",$urlOK=0, $urlKO=0, $NotificationURL=0){
        $result = RedsysWDG::setMyParameter($id,$amount,$language,$pDesc, $urlOK, $urlKO, $NotificationURL);
    ...
    public function getFormDataJsonDemo($id="5555",$amount="6000",$language="002", $pDesc="",$urlOK=0, $urlKO=0, $NotificationURL=0){
        $result = RedsysWDG::setMyParameterDemo($id,$amount,$language,$pDesc, $urlOK, $urlKO, $NotificationURL);
        return json_encode($result);
    }

    public function checkDataDemo($version,$datos,$signatureRecibida){
        $miObj = new RedsysAPI();
        $firma = $miObj->createMerchantSignatureNotif($miObj->params['KC_DEMO'],$datos);
        if ($firma === $signatureRecibida){
            return true;
        }else {
            return false;
    ...
    public function setMyParameter($id,$amount,$language,$pDesc, $urlOK=0, $urlKO=0,$NotificationURL=0){
        if(strlen($pDesc) > 125) $pDesc = substr($pDesc,0,122).'...';
        $miObj = new RedsysAPI();
        $miObj->params['URL']= $NotificationURL?$NotificationURL:$miObj->params['URL'];
        $miObj->params['URLOK'] = $urlOK?$urlOK:$miObj->params['URLOK'];
        $miObj->params['URLKO'] = $urlKO?$urlKO:$miObj->params['URLKO'];
    ...
    public function setMyParameterDemo($id,$amount,$language,$pDesc, $urlOK=0, $urlKO=0,$NotificationURL=0){
        if(strlen($pDesc) > 125) $pDesc = substr($pDesc,0,122).'...';
        $miObj = new RedsysAPI();
        $miObj->params['URL']= $NotificationURL?$NotificationURL:$miObj->params['URL'];
        $miObj->params['URLOK'] = $urlOK?$urlOK:$miObj->params['URLOK'];
        ...
        $version = $miObj->params['VERSION'];
        $kc = $miObj->params['KC_DEMO'];//Clave recuperada de CANALES
        // Se generan los parámetros de la petición
        $params = $miObj->createMerchantParameters();
        $signature = $miObj->createMerchantSignature($kc);

        $result = Array(
            'form_url'=>$miObj->params["FORM_URL_DEMO"],
            'ds_signatureversion'=>$version,
            'ds_signature'=>$signature,
            'ds_merchantparameters'=>$params
        );
